...
import 'erreProdukzioakPage.dart';


class ListErreProdukzioakPage extends StatefulWidget {

  ListErreProdukzioakPage(
      {Key? key})
      : super(key: key);

  @override
  State<ListErreProdukzioakPage> createState() => _ListErreProdukzioakPageState();
}

class _ListErreProdukzioakPageState extends State<ListErreProdukzioakPage> {
    ...
    bool delete = await showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('¿Estás seguro de que quieres borrar esta actividad de Erre produkzioak?'),
          actions: <Widget>[
            TextButton(
              child: Text('Cancelar'),
              onPressed: () {
  ...
  Future<void> _showEditActivityDialog(Activity activity) async {
    ...
    await showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text("Editar actividad de Erre produkzioak"),
...
Future<void> _showCreateActivityDialog(int nextActivityId) async {
  ...
  await showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: Text("Crear actividad de Erre produkzioak"),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
                            ...
                            children: [
                              ...
                              SizedBox(height: 8),
                                DataTable(
                                  ...
                                  rows: activitiesErreProdukzioak.where((activity) => activity.type!.id == type.id)
